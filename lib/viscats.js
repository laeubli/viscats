...
function translationSession ( csv_file_content, user_state_colors ) {
	// the visualisation of a single translation session
	// @param csv_file_content is a string with the contents of a CSV file to be visualised
	// @param state_color: a {state_name: '#color', ...} object that will override the default
	//                     color assignment.
	...
	var color_scale = function (index) {
		var scale = ['#588199', '#D6AA26', '#91204D', '#93A31C', '#6F5846', '#408156', '#30374F'];
		return scale[index%6] // maximum 7 state colors currently
	}; // the state colors will be dranw from here
	var state_color = {};
	...
	instance.getStateColors = function () {
		return state_color
	}
	...
	instance.appendTo = function ( selector ) {
		...
		// read CSV
		rows = d3.csv.parse(csv_file_content);
		...
		for (var key in rows) {
			...
			if (!state_color[row.state]) {
				if (user_state_colors && row.state in user_state_colors) {
					state_color[row.state] = user_state_colors[row.state]
				} else {
	...
	instance.appendStates = function ( container, data ) {
		...
		*/
		...
		function compare (a,b) { // helper function for object.sort()
			if (a.state < b.state)
				return -1;
			if (a.state > b.state)
				return 1;
